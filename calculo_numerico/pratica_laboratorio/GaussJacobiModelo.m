...
function GaussJacobiModelo
  % Metodo de Gauss-Jacobi para resolver sistema linear Ax=b
  clc
  format short
  ...
  new_x = zeros(n,1);     % solucao inicial (aproximada) em forma de matriz coluna
  old_x = zeros(n,1);     % solucao inicial (aproximada) em forma de matriz coluna
    ...
    for i = 1:n #percorrendo as linhas
      ...
      for j = 1:n #percorrendo as colunas
        if j ~= i #evita que o valor do coeficiente da própria variável que está sendo calculada seja considerado
          S = S + A(i,j)*old_x(j); #por exemplo, se estamos calculando x1, então S irá considerar os coeficientes de x2 e x3
        endif
      endfor
      new_x(i) = (b(i) - S) / A(i,i);
    endfor
    old_x = new_x; %atualizando old_x pensando na proxima iteracao

    %   Se norm(A*x-b,inf) < ee  % Testa se x eh solucao aproximada
    %     Se sim, fazer conv = true e abandonar iteracoes (usar "break")

    if norm(A*new_x - b, inf) < ee
      conv = true;
      break
    endif
  endfor
  ...
  if conv == true
    ...
    for i = 1:n
      fprintf("valor de x%d: %f\t", i, new_x(i));
    endfor
    fprintf("\n");
  else
